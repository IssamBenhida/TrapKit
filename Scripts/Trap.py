...
def banner():
	...
	print(custom_fig.renderText('                          T r a p k i T'))
...
def phishing_choise():
	...
	cprint("1 - Pre-built-in payload (suggested by TrapKit)\n ", 'magenta' ,attrs=['bold'])
...
def Liste_of_options():
	format = 0
	while True:
		cprint("\n1 - MP3 music (.mp3)", 'cyan' ,attrs=['bold'])
...
def input_file():
	...
	if (extension == "Invalid file choice"):
		...
		return "xls", "xls", "xls"
	file_path = input("\nset> The path to the file [!]absolute path: "); line()
...
def input_payload():
	pay_path = input("set> The payload path [absolute path]: "); line()
...
def replace(file,gb_text,out) :
	...
	for line in fin:
		...
		fout.write(line.replace('sbrdila', gb_text))
...
def special_excel(payload, Ip):
	...
	system("cat shell_out.txt")
	text = popen("cat shell_out.txt").read().split('\n')[0]
	system(('echo -n "{text}" > text.txt').format(text=text))
	...
	print("\n" + to_replace)
	cprint("\n[!] Inject the above base64 text into the creator tag in output/sheet.xls", 'yellow' ,attrs=['bold']);print("\n")
